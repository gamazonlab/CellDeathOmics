...
# Load data
unannotateddf <- read_csv('data/UKBBv3_SPrediXcanResults_PCDGenes_FINAL.csv')
...
# EHR trait Manhattan plots -------
 
# Establish cutoffs, labels, and breaks for Manhattan plots
# These are arbitrary and can be adjusted 
cutoff = 1.0e-5
labelcutoff=1.0e-5
...
df_ehr$LabelGenePheno <- ifelse(df_ehr$pvalue %in% mostsigGenePheno & df_ehr$pvalue < labelcutoff, paste0(df_ehr$gene_name, " - ", df_ehr$`Phenotype Description`, " - ", df_ehr$Tissue), NA)
df_ehr$LabelGeneCat <- ifelse(df_ehr$pvalue %in% mostsigGeneCat & df_ehr$pvalue < labelcutoff, paste0(df_ehr$gene_name, " - ", df_ehr$`Phenotype Description`, " - ", df_ehr$Tissue), NA)
df_ehr$LabelCat <- ifelse(df_ehr$pvalue %in% mostsigCat & df_ehr$pvalue < labelcutoff, paste0(df_ehr$gene_name, " - ", df_ehr$`Phenotype Description`, " - ", df_ehr$Tissue), NA)
...
# All associations plotted; only most significant gene/trait associations labeled
# Beyond arbitrary cutoff (p=1e-5)
# This is a bonus plot: not included in manuscript but less "busy" than the FDR<0.25 cutoff graph below: 
p_ehr_top <- ggplot(df_ehr, aes(x=Position, y = -log10(pvalue), color = Category, label=LabelGenePheno)) +
  geom_point(size = 1.5) +
  geom_text_repel(size=3, min.segment.length = 0.5, max.overlaps = Inf, nudge_y = 2, force=40, force_pull=0.1) +
  theme_classic() +
  ...
  labs(title="ICD10/FinnGen Top Gene/Trait Associations", y = "-log10(P-Values)")
#p_ehr_top
ggsave('figures/Manhattan_ICD10FinnGen_1e-5cutoff.tiff', plot=p_ehr_top, units="in", width=8.5, height=5)
...
p_ehr_fdr <- ggplot(df_ehr, aes(x=Position, y = -log10(pvalue), color = Category, label=LabelCat)) +
  geom_point(size = 1.5) +
  geom_text_repel(size=3, min.segment.length = 0.5, max.overlaps = Inf, nudge_y = 5, force=40, force_pull=0.2) +
  theme_classic() +
  theme(axis.title.x=element_blank(), axis.text.x=element_text(angle=45, hjust = 1),
        legend.position = "none", panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  ...
  scale_x_continuous(labels = xLabels, breaks = xBreaks) +
  geom_hline(yintercept=-log10(cutoff), color = "red")+
  labs(title="ICD10/FinnGen Top Gene/Trait Associations Per Trait Category", y = "-log10(P-Values)")
ggsave('figures/Manhattan_ICD10FinnGen_TopCategory_FDR25cutoff.tiff', plot=p_ehr_fdr, units="in", width=8.5, height=5)
...
df_lab <- filter(df_lab, !`Phenotype Description` %in% bloodtraits2remove)
...
cutoff = 1.520173e-06 # Bonferroni
labelcutoff=2e-20
...
p_lab_bonferroni <- ggplot(df_lab, aes(x=Position, y = -log10(pvalue), color = Category, label=LabelPheno)) +
  ...
  scale_x_continuous(labels = xLabels, breaks = xBreaks) +
  geom_hline(yintercept=-log10(cutoff), color = "red")+
  labs(title="Lab/Continuous Top Gene/Trait Associations Per Trait Category", y = "-log10(P-Values)")
p_lab_bonferroni
ggsave('figures/Manhattan_LabCont_PheWAS.tiff', plot=p_lab_bonferroni, units="in", width=8.5, height=5)
